...
for line in lines:
    ...
    cv2.imwrite("./out_text/test2.jpg",subimg)
    imread_img = cv2.imread("./out_text/test2.jpg",0)
    res = preprocessing(imread_img, minArea=10)
    for (j, w) in enumerate(res):
        (wordBox, wordImg) = w
        (x, y, w, h) = wordBox
        cv2.imwrite('./out_text/test2.jpg', wordImg)
    ...
    for char_width in range(0,col_width,char_plus_width):
        ...
        mp.predict("./check/renet_0509_fnt.h5", "test.jpg")
        if (mp.max<=0.75):
